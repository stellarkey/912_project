...
- `update(x, v)`：**修改单个元素**，把$A_x$修改为$v$。$O(logn)$。
  - 对一个区间整体操作：修改一个区间。
- `query(L, R)`：计算$\min_\{A_L, A_{L+1},...,A_R\}$（**区间求和**）。$O(logn)$。
...
- `insert(x)`：向Treap中插入`x`。
- `remove(x)`：从Treap中删除`x`。
- `find(x)`：查找`x`的位置并旋转到根节点 。
- `findkth(k)`：查找第`k`大元素。
- `PreSuf(x, f)`：查找`x`的前驱/后继。

```c++
struct SplayTree {
    struct node {
        /* tree[i].val：表示该点的值
         * tree[i].cnttree[i].cnt：表示该点在树上的出现次数
         * tree[i].siztree[i].siz：表示该点的子树大小，包括自己在内
         * tree[i].ftree[i].f：表示该点的爸爸（诶真乖）
         * tree[i].son[2]tree[i].son[2]：表示该点的两个儿子：son[0]son[0]左儿子，son[1]son[1]右儿子
        */
    ...
    int root, Size;
    SplayTree(){ insert(-INF); insert(INF);}    // 加入哨兵

    void update ( int x ) {
        /* 不同的题肯定会有添加或更改，比如最大值就应该写成
