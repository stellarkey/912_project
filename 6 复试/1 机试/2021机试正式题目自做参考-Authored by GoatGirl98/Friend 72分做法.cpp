// 72分做法，按照数据范围应该过不掉最后一个subtask，但是卡常到极限也有可能
// 100分做法目测是三元环+容斥，但是我懒得做了QAQ
#include <stdio.h>
#include <string.h>
#include <vector>
...
int main()
{
    scanf("%d%d", &n, &m);
    for (int i = 1; i <= m; ++i)
        scanf("%d%d", &u[i], &v[i]), g[u[i]].insert(v[i]), g[v[i]].insert(u[i]);
    ...
    for (int i = 1; i <= m; ++i)
    {
        for (int j = 1; j <= n; ++j)
        {
            if (j == u[i] || j == v[i])
                continue;
            int k = ((g[j].count(u[i]) != 0) << 1) | (g[j].count(v[i]) != 0);
            switch (k)
            {
            case 0:
