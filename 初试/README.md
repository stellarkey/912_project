...
> **<font color=blue>假</font>**。一顿`fork()`。
...
(3) 以太网中最短帧长1000bit，两点相距100m,数据在光纤中的传播速率为2*10^8m/s,问最大发送速率
A.1Gb/s B.2Gb/s C.100Mb/s 4.200Mb/s

> 最大数据传输率就是将所有等待ACK的时间都利用起来。
>
> RTT=`2*100/(2*10^8)`=1us。
>
> 

##### 分组转发

(5) 计算⼀个计算分组时间，两个转发加两个传播
...
(6) TCP中，拥塞窗口大小W（窗口为m时发⽣拥塞），最大发送段长MSS，给RTT，发送⽅有⾜够多的数据要发，求算出平均速率近似是多少
