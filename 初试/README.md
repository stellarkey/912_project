...
> 不妨认为这里使用快重传和快恢复策略。（MSS=最大报文段长度）
>
> 1）不超时，ACK=8KB<16KB，处于慢开始阶段，拥塞窗口的大小=8KB。
>
> 2）不超时，ACK=31KB>16KB，处于拥塞避免阶段，拥塞窗口加法增大，拥塞窗口的大小=31KB。
>
> 3）超时，不妨设设此时拥塞窗口为32KB
>
> - 由快重传，1RTT后，拥塞窗口为16KB（腰斩）
> - 由快恢复，进入拥塞避免阶段（加法增大）
