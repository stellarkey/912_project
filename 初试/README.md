...
MIPS 处理器内存延迟 10ns， ALU 延迟 6ns，寄存器 3ns，输⼊延迟 1ns，流⽔线寄存器以及多周期锁存器输出延迟为 2ns，要有计算过程，
...
> （1）**IF**=读指令内存=内存延迟=10ns；**ID**=读寄存器=3ns；**EXE**=ALU延迟=6ns；**MEM**=写数据内存=内存延迟=10ns；**WB**=写寄存器=3ns。各条指令延迟如下：
>
> - `addu rs rt rd`：IF+ID+EXE+WB=
> - `subu rs rt rd`：IF+ID+EXE+WB=
> - `ori rs rt imm`：IF+ID+EXE+WB=
> - `lw rs rt imm`：IF+ID+EXE+MEM+WB=
> - `sw rs rt imm`：IF+ID+EXE+MEM=
> - `beq rs rt imm`：IF+ID+EXE=
> - `j target`：IF=

##### Version 2

**pc输入2ns**，寄存器堆输入1ns，输出2ns（寄存器堆在一个时钟周期内可以读取两个寄存器，或者写入一个寄存器），ALU延迟6ns，内存访问延迟10ns，组成流水线用来缓存的缓存器2ns。部分mips指令如下：
...
> > 本题在期末复习题PPT中出现。
> >
> > ![1571218066106](README/1571218066106.png)
...
> 大概如下图？
>
> ![1571393785998](README/1571393785998.png)
> $$
...
> ![1571396369250](README/1571396369250.png)
>
> 如上图，则
> $$
...
> >  本体是典例中的原题：
> >
> > ![1571394735865](README/1571394735865.png)
> >
> > ![1571394824723](README/1571394824723.png)
> >
> > ![1571394839120](README/1571394839120.png)
...
> >
> > ![1571218282429](README/1571218282429.png)
...
> 不会。copy on write机制下，它们共享页表，只有当对某页发生写操作时，才引发Page Fault单独复制并分配该页给操作进程。
...
> 我猜是跟2018年一样的AVL树。那么，根据之前的结论，显然在插入1023个节点以后，形成高度为9的满树。此时再插入剩下的节点=993
